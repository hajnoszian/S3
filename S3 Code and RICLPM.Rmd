...
```{r}
df$pprMean <- rowMeans(subset(df, select = c(a_p2ppr1,a_p2ppr2, a_p2ppr3, a_p2ppr4)), na.rm = T) #need the rowMeans() call since mean() can't handle a row entirely of NAs, like we have every now and then
df$kmMean <- rowMeans(subset(df, select = c(a_p2km1,a_p2km2, a_p2km3, a_p2km4, a_p2km5, a_p2km6)), na.rm = T)
df$paMean <- rowMeans(subset(df, select = c(a_p2pa1,a_p2pa2, a_p2pa3, a_p2pa4, a_p2pa5)), na.rm = T)
...
#Rel Sat
summary(lm(data = df,
           a_p2relsat ~ Institution)) #Southwestern is a tad higher it seems
ggplot(data = df, aes(x = Institution, y = a_p2relsat)) +
