...
o1 <- df |> 
  ...
  group_by(ID, Day) #1318 observations of KM evokers. 522 observations total, means people were reporting on average ~2.5 evokers per day
#Remember, people could report more than one KM evoker per each day
all(o1$Mark == o1$value)#This is false, which is to be expected since I've cut 2 KM items and thus 2 opportunities for an evoker. Makes it a bit more complicated, which is one argument to use the above plotting since they are 100% linked together. 
...
```
Yeah, even the most generous, basic CFA PPR measurement model looks funky here. It's not doing great by itself. Arguably, ppr4 is not doing great relative to the others but it's still looking pretty good tbh.
...
```{r}
anova(weak.m1.fit, strong.m1.fit)
```
...
base.m2D <- '
  ...
  # Create between factors (RIs)
  RIkm =~ 1*Fkm1 + 1*Fkm2 + 1*Fkm3 + 1*Fkm4 + 1*Fkm5 + 1*Fkm6 + gend + rellength
  RIppr =~ 1*Fppr1 + 1*Fppr2 + 1*Fppr3 + 1*Fppr4 + 1*Fppr5 + 1*Fppr6 + gend + rellength
...
strongInvar.m1.fit <- cfa(weakInvar.m1,
                   data = dfSEM,
                   std.lv = T,
                   missing = "ML"
                   )
