...
```{r}
o1 <- df
levels(o1$Institution) <- c("Locale 1", "Locale 2", "Locale 3")

p1<- ggplot(data = o1, aes(x = day, y = ppr1, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") + 
  facet_wrap(~Institution)
p2<- ggplot(data = o1, aes(x = day, y = ppr2, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
p3<- ggplot(data = o1, aes(x = day, y = ppr3, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
p4<- ggplot(data = o1, aes(x = day, y = ppr4, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
...
p1<- ggplot(data = o1, aes(x = day, y = km1, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
#p2<- ggplot(data = df, aes(x = day, y = km2, group = day)) +
...
#  stat_summary(fun.y = "mean") +
#  facet_wrap(~Institution)
p4<- ggplot(data = o1, aes(x = day, y = km4, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
p5<- ggplot(data = o1, aes(x = day, y = km5, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
p6<- ggplot(data = o1, aes(x = day, y = km6, group = day)) +
  geom_boxplot(alpha = .5) +
  stat_summary(fun.y = "mean") +
  facet_wrap(~Institution)
...
Try out the strong model, since that's the best model so far.
```{r}
strongInvar.m1 <- '
...
```{r}
anova(strong.m3.fit, strongInvar.m1.fit)
```
##### Effects Invariance Per Outcome  (MAIN Model)
In this one, we'll fix the autoregressives--Km to KM, PPR to PPR--rather than fix them wholesale like above
```{r}
strongInvar.m2 <- '
  ...
  # Specify lagged effects between within-person centered latent variables (constrained)
  WFkm2 ~ s*WFkm1 + tt*WFppr1
  WFppr2 ~ t*WFppr1 + ss*WFkm1
  WFkm3 ~ s*WFkm2 + tt*WFppr2
  WFppr3 ~ t*WFppr2 + ss*WFkm2
  WFkm4 ~ s*WFkm3 + tt*WFppr3
  WFppr4 ~ t*WFppr3 + ss*WFkm3
  WFkm5 ~ s*WFkm4 + tt*WFppr4
  WFppr5 ~ t*WFppr4 + ss*WFkm4
  WFkm6 ~ s*WFkm5 + tt*WFppr5
  WFppr6 ~ t*WFppr5 + ss*WFkm5
...
strongInvar.m2.fit <- cfa(strongInvar.m2,
                   data = dfSEM,
                   std.lv = T,
                   missing = "ML"
                   )
summary(strongInvar.m2.fit, fit.measures = T, standardized =T, ci = T)
```
```{r}
anova(strongInvar.m2.fit, strong.m3.fit)
```
TECHNICALLY this model is not statistically significantly different than the m3 model, which tells me that I CAN use this as my main model actually. I.e. it is fair to say that KM -> KM is fixed along the period, and that PPR -> PPR is fixed along the period 

```{r}
modindices(strongInvar.m2.fit, sort = T)
```
